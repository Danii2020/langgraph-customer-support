...
- **`SourceBucket`** — S3 bucket. EventBridge notifications are enabled but the eval pipeline does not consume the S3 notifications directly; it subscribes to the KB's ingestion log via CloudWatch Logs (see eval-pipeline section below).
...
Stack output `KnowledgeBaseId` is consumed by `.env` key `KNOWLEDGE_BASE_ID` (LangGraph app) and `evaluation/samconfig.toml` `parameter_overrides` key `KnowledgeBaseId` (eval pipeline). Cross-stack coupling is human-mediated copy/paste. Both stacks must be deployed to the same region — the eval stack's `AWS::Logs::DeliverySource` resolves the KB ARN region-locally, and same-region routing keeps the CloudWatch Logs subscription latency under a few seconds.
...
- **Four Lambdas** (`lambdas/start_eval_job`, `check_eval_status`, `parse_eval_results`, `format_notification`) — each has a thin `handler.py` and is imported by tests directly via `evaluation/tests/`. `start_eval_job` dispatches on `eval_type` ∈ {`RETRIEVE_AND_GENERATE`, `RETRIEVE_ONLY`}; the RAG branch builds a `retrieveAndGenerateConfig` (5 generation metrics, `taskType: "Summarization"`, generator model + prompt template), the retrieve-only branch builds a `retrieveConfig` (2 context metrics, `taskType: "General"`, no generator, no prompt). `parse_eval_results` accepts a `thresholds_subkey` parameter (default `"retrieve_and_generate"`) so each branch reads its own thresholds block from S3. `format_notification` is the single source of truth for the combined verdict and the email body — it consumes the Parallel state's full output (both branch verdicts, or `$.error` on pre-completion failures) and returns `{subject, message, passed}` with a plain-text per-metric summary, per-branch PASS/FAIL markers, and a contextual "next steps" hint keyed on which branch failed.
- **A Step Functions state machine** with a **`Parallel` state** that runs both evals concurrently. Branch A (RAG) polls for ~10–15 min; Branch B (retrieval) polls for ~3–5 min. Initial waits differ per branch: RAG 300 s, retrieval 120 s, so polling Lambdas don't fire before either job is close to done. Total runtime ≈ max(rag, retrieval), not sum. After the Parallel state (via its `Next` *or* its `Catch: States.ALL` — both paths route here) the flow is dead simple: `FormatNotification` (Task → the format-notification Lambda) → `PublishNotification` (Task → `sns:publish`) → END. The verdict AND-merge logic — *both* branches must pass every metric — lives inside the Lambda; ASL has no Choice-level boolean AND across paths, but the Lambda trivially expresses it.
- **Two S3 buckets** (`EvalBucket` for datasets + thresholds, `ResultsBucket` for Bedrock eval output) — provisioned by the stack itself with auto-generated globally-unique names. A `SeedEvalAssetsCustomResource` Lambda uploads four seed files on stack Create (`evaluation_dataset.jsonl` + `retrieval_eval_dataset.jsonl` + `thresholds.json` + `retrieval_thresholds.json`) and empties both buckets on stack Delete. The SHA-256 `SeedAssetsHash` property on the custom resource is auto-maintained by `prepare_lambda_assets.py` so edits to any of the four seed files trigger a re-upload on the next `sam deploy`.
- **Two triggers for the state machine**:
  1. **`PromptVersionPublishedRule`** (EventBridge, CloudTrail-based): `source: aws.bedrock`, `detail-type: "AWS API Call via CloudTrail"`, `eventName: CreatePromptVersion`, filtered by `requestParameters.promptIdentifier == PromptResourceId`. Note: `CreatePromptVersion` is invoked via the boto3 `bedrock-agent` client but CloudTrail logs it under `eventSource: bedrock.amazonaws.com` (hence the `aws.bedrock` source). `AWS API Call via CloudTrail` events only reach EventBridge when a CloudTrail trail captures them in the region, so the stack also provisions `EventBridgeManagementTrail` + `TrailLogBucket` + its bucket policy.
  2. **KB ingestion log subscription** — Bedrock KBs do **not** emit a native EventBridge event when an ingestion job finishes (the previous `KbSyncCompletionRule` filtering on `detail-type: "Bedrock Knowledge Base Data Source Sync"` matched nothing — that detail-type does not exist). Instead the stack provisions `AWS::Logs::DeliverySource` (`LogType: APPLICATION_LOGS`, `ResourceArn: <KB ARN>`) + `AWS::Logs::DeliveryDestination` (a `/aws/vendedlogs/bedrock/knowledge-base/...` log group) + `AWS::Logs::Delivery` to wire the KB's ingestion log into CloudWatch Logs, then attaches a `SubscriptionFilter` with pattern `{ $.event.ingestion_job_status = "COMPLETE" }` to `KbIngestionCompleteFunction`. The Lambda decodes the gzip-base64 subscription payload, defensively re-checks status + `knowledge_base_arn`, and calls `stepfunctions:StartExecution` with the same input JSON the old rule was supposed to send (so the state machine's input handling is unchanged). Fires for every path that ends in `StartIngestionJob` — console "Sync", CLI, `seed_and_ingest.py`, CI.

  The KB prompt template lives in Bedrock Prompt Management (created by `create_eval_prompt.py`), not in S3.

The pipeline calls `bedrock.create_evaluation_job` with `applicationType="RagEvaluation"` in **both** branches. The RAG branch uses the five generation metrics — `Faithfulness`, `Correctness`, `Completeness`, `Helpfulness`, `LogicalCoherence` — scored against `evaluation/config/thresholds.json["retrieve_and_generate"]`. The retrieve-only branch uses the two retrieval metrics — `ContextRelevance`, `ContextCoverage` — scored against `evaluation/config/retrieval_thresholds.json["retrieve_only"]`. `parse_eval_results` averages each metric across `conversationTurns[*].results[*]` and CamelCase-normalizes names (`Builtin.LogicalCoherence` → `logical_coherence`, `Builtin.ContextRelevance` → `context_relevance`). A metric passes iff `score >= threshold`, and the overall verdict passes iff **both branches** pass every metric.

The SNS email body is **not** a JSON dump — `format_notification` renders a fixed-width plain-text report: per-branch PASS/FAIL header, an aligned `metric / score / threshold / result` table, the failing-metric list, both Bedrock job ARNs, and a contextual remediation hint ("RAG metrics low → generator/prompt issue", "retrieval metrics low → chunking/embedding/top-K/missing docs"). The subject line encodes the per-branch verdict (`Eval Pipeline: FAIL (RAG passed, Retrieval failed)`) so the inbox view is informative without opening the email. The Lambda also handles the pre-completion Parallel-Catch path (Bedrock throttling, IAM errors, etc.) with a distinct "pipeline error" subject and a message that surfaces the `Cause` from the Step Functions Catch payload.
