...
RETRIEVAL_ONLY_EVENT = {**BASE_EVENT, "eval_type": "RETRIEVAL_ONLY"}
RAG_EVENT = {
    **BASE_EVENT,
    "eval_type": "RETRIEVE_AND_GENERATE",
    "eval_config": {
        **BASE_EVENT["eval_config"],
...
# ---------------------------------------------------------------------------
# Retrieval-only job creation
# ---------------------------------------------------------------------------

class TestStartEvalJobRetrievalOnly:

    def test_returns_job_arn(self):
        expected_arn = "arn:aws:bedrock:us-east-2:123:evaluation-job/ro-job-001"
    ...
    def test_job_name_contains_retrieval_only(self):
        mock_bedrock = make_mock_bedrock()
        with patch.object(_mod.boto3, "client", return_value=mock_bedrock):
            handler(RETRIEVAL_ONLY_EVENT, None)
        ...
        call_kwargs = mock_bedrock.create_evaluation_job.call_args[1]
        assert (
            "retrieval-only" in call_kwargs["jobName"].lower()
            or "retrieval_only" in call_kwargs["jobName"].lower()
        )
...
class TestStartEvalJobErrors:
    ...
    def test_raises_key_error_when_knowledge_base_id_missing(self):
        event = {"eval_type": "RETRIEVAL_ONLY", "eval_config": BASE_EVENT["eval_config"]}
        with pytest.raises(KeyError, match="knowledge_base_id"):
            handler(event, None)

    def test_raises_value_error_for_invalid_eval_type(self):
        event = {**RETRIEVAL_ONLY_EVENT, "eval_type": "INVALID_TYPE"}
